...
# -- Resume support --
# `pkg upgrade` can replace core binaries/libs (bash, libc++, ncurses)
# out from under the running process, which gets it killed by the OS
# with no error message - just dumped back to the prompt. STATE_FILE
# remembers the last fully-completed step so re-running the script
# after that happens skips finished work instead of starting over.
...
# NOTE: "pkg upgrade" here can replace bash/libc++/ncurses mid-flight
# and get this very process killed by the OS with no error output -
# you'll just land back at the prompt. If that happens, simply rerun
# this script; step 1 (and this step, once it finishes) will be
# skipped via the resume check above/below.
